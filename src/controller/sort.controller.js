const { newAdded, popularProducts } = require("../models/sort")

const renderLoggedIn = (data, dataUser) => {
    let renderData = ''
    data.forEach(ele => {
        renderData += `
...
module.exports = {
    httpNewAdded: async (req, res) => {
        ...
        if (data) {
            if (dataUser) {
                renderData = renderLoggedIn(data, dataUser)
            } else {
                renderData = renderNotLoggedIn(data)
            }
    ...
    httpPopularProducts: async (req, res) => {
        let renderData = ''
        const dataUser = JSON.parse(req.query.dataUser)
        let [data] = await popularProducts().catch(err => err)
        ...
        return res.status(400).json({ ok: false, data: 'Error finding Popular Products' })
    }
}