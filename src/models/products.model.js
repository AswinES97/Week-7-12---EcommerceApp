...
const deletImageFromCloud = async (pId) => {
  await getSingleProduct(pId).then(async (data) => {
...
module.exports = {
  ...
  getAllProducts: (skip = 0) => {
    return new Promise(async (resolve, reject) => {
      await product
        .find({ active: true, "quantity.quantity": { $gt: 0 } })
        .skip(skip)
        .limit(10)
        .then((res) => JSON.parse(JSON.stringify(res)))
        .then((data) => resolve(data))
  ...
  addNewProduct: (productData, imageLink) => {
    return new Promise(async (resolve, reject) => {
      ...
      const slug = slugify(`${productData.name} ${productData.brand}`, {
        lower: true,
        replacement: "_",
      });
  ...
  deleteProduct: (pId) => {
    return new Promise(async (resolve, reject) => {
      ...
      await product.deleteOne({ pId }).then((data) => {
        if (data.deletedCount == 1) resolve(true);
        else resolve(false);
      });
    });
  },
  ...
  updateProductCategory: async (data) => {
    try {
      await product
        .updateMany(
          { categoryType: data.previousCategoryType },
          {
            $set: {
  ...
  shopCategoryFilter: async (data) => {
    try {
      return await product
        .find({ category: data.category, categoryType: data.categoryType })
        .then((res) => JSON.parse(JSON.stringify(res)));
    } catch (err) {
      console.log(err);
      return Promise.reject(false);
